...
═══════════════════════════════════════════════════════════════════
ALWAYS include the data timestamp in your response so users know data freshness:
• For prices: "Data as of: [last_updated from tool]"
• For history: "Latest data point: [most recent date]"
• For news: "Published: [published_at]"
• For funds: "NAV as of: [nav_date or last_updated]"

EXAMPLE FORMAT:
"Al Rajhi (1120) is trading at SAR 97.10 (+2.2%)
📅 Data as of: Dec 31, 2025 12:24 PM"

If last_updated is missing, say: "⚠️ Data freshness: Unknown"
...
async def get_market_summary():
    ...
    try:
        ...
        timestamp_query = "SELECT MAX(last_updated) as data_as_of FROM market_tickers"
        ts_row = await db.fetch_one(timestamp_query)
        data_as_of = str(ts_row['data_as_of']) if ts_row and ts_row['data_as_of'] else None
...
async def get_fund_details(fund_name: str):
    ...
    query = """
        SELECT fund_id, fund_name, fund_manager, fund_type, currency, 
...
async def get_fund_performance(fund_name: str, period: str = "1y"):
    ...
    # First find the fund
    fund_query = "SELECT fund_id, fund_name FROM mutual_funds WHERE fund_name ILIKE $1 LIMIT 1"
    try:
