...
export default function MarketTicker() {
    ...
    const duplicatedTickers = [...displayTickers, ...displayTickers, ...displayTickers];
            ...
            <div className="flex whitespace-nowrap">
                ...
                >
                    {duplicatedTickers.map((item, index) => (
                        <div key={`${item.symbol}-${index}`} className="flex items-center space-x-3 px-6 border-r border-slate-100 last:border-0 hover:bg-slate-50 cursor-pointer transition-colors">
                            <span className="font-bold text-sm text-slate-800 font-sans">{item.symbol}</span>
