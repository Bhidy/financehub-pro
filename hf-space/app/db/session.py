...
class Database:
    ...
    async def connect(self):
        if not self._pool:
            ...
            try:
                ...
                # CRITICAL: statement_cache_size=0 required for Supabase pgbouncer compatibility
    ...
    async def executemany(self, query: str, args_list):
        """Execute same query with multiple sets of arguments - for batch inserts"""
        if not self._pool: return
        async with self._pool.acquire() as conn:
            for args in args_list:
                try:
                    ...
                    pass  # Skip duplicates silently (ON CONFLICT DO NOTHING handles this)
