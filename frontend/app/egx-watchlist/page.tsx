...
export default function WatchlistPage() {
    ...
    return (
        <div className="min-h-screen bg-[#F1F5F9] dark:bg-[#1A222C] text-slate-900 dark:text-white font-sans pb-20">
            {/* HEADER */}
            <header className="bg-white dark:bg-[#1A222C] border-b border-slate-200 dark:border-[#2E3A47] sticky top-0 z-40">
                <div className="max-w-7xl mx-auto px-4 py-6">
                    <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
                        <div>
                            <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
                                <Eye className="w-8 h-8 text-[#3C50E0]" />
                                Smart Watchlist
                            </h1>
                        ...
                        <div className="flex items-center gap-3 w-full md:w-auto">
                            <div className="relative group w-full md:w-64">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-[#3C50E0] transition-colors" />
                                <input
                                    type="text"
                                    placeholder="Search Ticker..."
                                    value={search}
                                    onChange={e => setSearch(e.target.value)}
                                    className="w-full pl-10 pr-4 py-2.5 bg-[#F1F5F9] dark:bg-[#1A222C] border border-slate-200 dark:border-[#2E3A47] focus:bg-white dark:focus:bg-[#24303F] focus:border-[#3C50E0] focus:ring-1 focus:ring-[#3C50E0] rounded-md text-sm font-medium transition-all outline-none placeholder:text-slate-400"
                                />
                            </div>
                            <button className="p-2.5 bg-[#F1F5F9] dark:bg-[#1A222C] hover:bg-slate-200 dark:hover:bg-[#24303F] border border-slate-200 dark:border-[#2E3A47] rounded-md transition-colors">
            ...
            <main className="max-w-7xl mx-auto px-4 py-8 animate-in fade-in duration-500">
                <div className="premium-glass rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-[#F1F5F9] dark:bg-[#1A222C] border-b border-slate-200 dark:border-[#2E3A47] text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">
                                ...
                                {filtered.length === 0 ? (
                                    ...
                                    filtered.map((stock, i) => {
                                        ...
                                        return (
                                            <tr
                                                key={stock.isin}
                                                className="border-b border-slate-200 dark:border-[#2E3A47] hover:bg-slate-50 dark:hover:bg-[#1A222C] transition-colors group cursor-pointer"
                                            ...
                                            >
                                                <td className="p-5">
                                                    <Link href={`/egx/${stock.symbol}`} className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
                                                        <div className="w-10 h-10 rounded-md bg-[#F1F5F9] dark:bg-[#1A222C] flex items-center justify-center text-xs font-bold text-[#3C50E0] border border-slate-200 dark:border-[#2E3A47] transition-colors">
                                                            {stock.symbol ? stock.symbol.substring(0, 2) : '??'}
                                                        </div>
                                                        <div>
                                                            <div className="font-bold text-slate-900 dark:text-white group-hover:text-[#3C50E0] transition-colors">{stock.symbol}</div>
                                                ...
                                                <td className="p-5 text-center">
                                                    <Link href={`/egx/${stock.symbol}`} className="inline-flex p-2 text-slate-400 hover:text-[#3C50E0] hover:bg-teal-50 dark:hover:bg-teal-500/10 rounded-lg transition-all">
