...
import { PieChart, Pie, Cell, ResponsiveContainer, RadarChart, PolarGrid, PolarAngleAxis, Radar, Tooltip, AreaChart, Area, XAxis, YAxis, Legend } from 'recharts';
import { ArrowUpRight, ArrowDownRight, TrendingUp, Activity, BarChart2, DollarSign, PieChart as PieIcon, List, Building2, Calendar, FileText } from 'lucide-react';
...
const COLORS = { blue: '#3B82F6', green: '#10B981', amber: '#F59E0B', purple: '#8B5CF6', rose: '#F43F5E', teal: '#14B8A6' };
...
const MetricCard = ({ label, value, sub, color = 'blue', icon: Icon }: any) => {
...
const GaugeRing = ({ value, label, score }: any) => {
    ...
    return (
        <div className="flex flex-col items-center bg-white/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-100 hover:shadow-xl transition-all">
...
export default function EGXStockProfilePage() {
    ...
    if (loading) return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-slate-500 font-bold animate-pulse">Loading Premium Data...</p>
            </div>
        </div>
    );
    ...
    const TrendIcon = isPositive ? TrendingUp : ArrowDownRight;
                ...
                {tab === 'overview' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            <MetricCard label="Market Cap" value={fmtLarge(stock.market_cap)} sub="EGP" color="blue" />
                            <MetricCard label="P/E Ratio" value={fmt(stats?.pe_ratio, 2)} sub="Trailing 12M" color="purple" />
                            <MetricCard label="Div Yield" value={fmtPct(stats?.dividend_yield)} sub="Annualized" color="green" />
                            <MetricCard label="Volume" value={fmtLarge(stock.volume)} sub="Daily Avg" color="amber" />
                        </div>

                        {/* Scores & Gauges */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <GaugeRing score={stats?.pe_ratio < 15 ? 85 : 60} value="Valuation Score" label="Attractiveness" />
                            <GaugeRing score={stats?.roe > 0.2 ? 92 : 75} value="Profitability" label="Efficiency" />
                            <GaugeRing score={stats?.current_ratio > 1.5 ? 88 : 65} value="Financial Health" label="Safety" />
                        </div>
                    </div>
                )}
                ...
                {tab === 'statistics' && stats && (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in duration-500">
                        {[
                            ...
                            {
                                title: "Profitability & Returns", icon: Activity, color: "green",
                                items: [
                            ...
                            {
                                title: "Financial Health", icon: Building2, color: "amber",
                                items: [
