...
export default function ScreenerPage() {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30 dark:from-[#0B1121] dark:via-[#0F1629] dark:to-[#0B1121]">
            {/* Premium Header */}
            ...
            <div className="max-w-[1800px] mx-auto px-6 py-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6 -mt-10">
                    {[
                        ...
                        <div key={i} className="bg-white dark:bg-[#1A1F2E] rounded-2xl border border-slate-100 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-black/20 p-5 flex items-center gap-4">
                            <div className={clsx(
                ...
                {/* Filters Bar */}
                <div className="bg-white dark:bg-[#1A1F2E] rounded-2xl border border-slate-100 dark:border-white/5 shadow-lg shadow-slate-100/50 dark:shadow-black/20 p-5 mb-6">
                    <div className="flex flex-wrap items-end gap-4">
                        {/* Sector */}
                        <div className="flex-1 min-w-[200px]">
                            <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">Sector</label>
                            <select
                                className="w-full p-3 border border-slate-200 dark:border-white/10 rounded-xl text-sm font-medium bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                        ...
                        {/* Price Range */}
                        <div className="flex-1 min-w-[200px]">
                            <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">Price Range (SAR)</label>
                            <div className="flex gap-2 items-center">
                                <input
                                    type="number"
                                    className="flex-1 p-3 border border-slate-200 dark:border-white/10 rounded-xl text-sm font-medium bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                                    placeholder="Min"
                                    ...
                                    <button
                                        ...
                                        className={clsx(
                                            ...
                                            filters.sort_by === field.key
                                                ? field.color === 'blue' ? "bg-blue-500 border-blue-500 text-white shadow-lg shadow-blue-200 dark:shadow-blue-900/20"
                                                    : field.color === 'teal' ? "bg-teal-500 border-teal-500 text-white shadow-lg shadow-teal-200 dark:shadow-teal-900/20"
                                                        : field.color === 'emerald' ? "bg-emerald-500 border-emerald-500 text-white shadow-lg shadow-emerald-200 dark:shadow-emerald-900/20"
                                                            : "bg-orange-500 border-orange-500 text-white shadow-lg shadow-orange-200 dark:shadow-orange-900/20"
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={toggleOrder}
                                className="p-3 bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 rounded-xl text-slate-600 dark:text-slate-300 transition-all"
                            >
                ...
                {/* Results Table */}
                <div className="bg-white dark:bg-[#1A1F2E] rounded-2xl border border-slate-100 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-black/20 overflow-hidden">
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-gradient-to-r from-slate-50 to-slate-100 dark:from-white/5 dark:to-white/5 border-b border-slate-200 dark:border-white/5">
                                ...
                                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Sector</th>
                                <th className="px-6 py-4 text-right text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Price</th>
                                <th className="px-6 py-4 text-right text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Change</th>
                                <th className="px-6 py-4 text-right text-xs font-bold text-blue-500 dark:text-blue-400 uppercase tracking-wider">Volume</th>
                                <th className="px-6 py-4 text-right text-xs font-bold text-orange-500 dark:text-orange-400 uppercase tracking-wider">P/E</th>
                                <th className="px-6 py-4 text-right text-xs font-bold text-teal-500 dark:text-teal-400 uppercase tracking-wider">Mkt Cap</th>
                            </tr>
                        </thead>
                            ...
                            {isLoading ? (
                                ...
                                return (
                                    <tr key={r.symbol} className={clsx(
                                        "hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors group cursor-pointer",
                                        i % 2 === 0 ? "bg-white dark:bg-transparent" : "bg-slate-50/30 dark:bg-white/5"
                                    )}>
                                        <td className="px-6 py-4">
                                            <Link href={profileUrl} className="flex items-center gap-3">
                                                ...
                                                <span className="font-bold text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{r.symbol}</span>
                                            </Link>
                                        </td>
                                        <td className="px-6 py-4 font-medium text-slate-700 dark:text-slate-300 max-w-[200px] truncate">{r.name_en}</td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <span className={clsx(
                                                ...
                                                Number(r.change_percent) >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"
                                            )}>
                                                {Number(r.change_percent) >= 0
                                        ...
                                        <td className="px-6 py-4 text-right font-mono text-orange-600 dark:text-orange-400 font-bold">
                                            {r.pe_ratio || "—"}
                                        </td>
                                        <td className="px-6 py-4 text-right font-mono text-teal-600 dark:text-teal-400 font-bold">
                                            {r.market_cap_b || "—"}
                                        </td>
                                    </tr>
                                )
