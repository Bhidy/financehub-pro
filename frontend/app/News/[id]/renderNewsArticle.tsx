...
export async function renderNewsArticle(idParam: string, tree: SiteLang) {
    ...
    // Headline-only recent list (no article_body): one indexed read of a few
    // kilobytes. This block used to fetch 60 full rows with bodies per render,
    // and the 2,500-row cached window tried in between cost ~0.3 s of Data Cache
    // payload per article (news read-path audit, 2026-09-05).
    const latest = (await getLatestNewsLight(60))
        .filter((n) => Number(n.id) !== article.id && newsLang(n) === lang)
        .slice(0, 6);
