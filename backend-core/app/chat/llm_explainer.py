...
class LLMExplainerService:
    ...
    async def generate_narrative(
        ...
        f"   - فقرة 2 (نقاط مجمعة - Bullet Points): 3 إلى 5 نقاط تستخلص المؤشرات ومقارنتها بمتوسطات القطاع. يجب تقدير الفرق نسبياً بوضوح.\n"
        f"     ⚠️ CRITICAL FORMATTING: يجب ترك مسافة سطرين فارغين (`\\n\\n`) قبل بدء قائمة النقاط. اكتب كل نقطة في سطر جديد بتنسيق `- **عنوان عريض**: وصف`.\n"
        f"   - فقرة 3: استنتاج للمحركات الأساسية، مفصولة بسطرين فارغين (`\\n\\n`). ما هي عوامل الربحية أو النمو التي تبرر هذا التقييم؟\n\n"
        ...
        f"   - Section 2 (Bullet Points): 3-5 items on Valuation Setup, Peer Context, and Fundamental Drivers. Use exact numbers from the context. You MUST quantify relative differences (e.g., 'trading at a X% discount to peers').\n"
        f"     ⚠️ CRITICAL FORMATTING: You MUST insert a double line break (`\\n\\n`) before starting the bullet list. Format each bullet on a NEW LINE starting with `- **Bold Title**: Description`.\n"
        f"   - Section 3 (Paragraph): Strategic Conclusion based on the provided metrics and the user's question, separated by a double line break (`\\n\\n`).\n"
