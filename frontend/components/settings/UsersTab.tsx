...
export default function UsersTab() {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Data Table */}
            <div className="bg-white dark:bg-[#151925] rounded-xl border border-slate-200 dark:border-white/5 shadow-sm overflow-hidden">
                                    ...
                                    >
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 dark:from-white/10 dark:to-white/5 flex items-center justify-center text-slate-500 dark:text-slate-300 font-bold text-sm">
                                                    {user.full_name ? user.full_name[0].toUpperCase() : user.email[0].toUpperCase()}
                                                </div>
                                                <div>
                                                    <div className="font-semibold text-slate-900 dark:text-white">{user.full_name || "No Name"}</div>
                                        ...
                                        <td className="px-6 py-4">
                                            {user.phone ? (
                                                <div className="flex items-center gap-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-white/5 px-2 py-1 rounded-lg w-fit border border-slate-100 dark:border-white/5">
                ...
                {/* Pagination */}
                {totalPages > 1 && (
                    <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 flex items-center justify-between bg-white dark:bg-[#151925]">
                        <button
                            disabled={page === 0}
            ...
            {isResetOpen && selectedUser && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <motion.div
                        ...
                        className="bg-white dark:bg-[#1A1F2E] rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-white/5"
                    >
                        <div className="p-6 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
                            ...
                            <button
                                ...
                                className="p-1 rounded-full hover:bg-slate-100 dark:hover:bg-white/10 text-slate-400 dark:text-slate-500 transition-colors"
                            >
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <div className="p-6 space-y-4">
                            <div className="bg-slate-50 dark:bg-white/5 p-3 rounded-xl border border-slate-100 dark:border-white/5 flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-white dark:bg-white/10 border border-slate-200 dark:border-white/5 flex items-center justify-center text-slate-500 dark:text-slate-300 font-bold text-xs">
                                    {selectedUser.full_name ? selectedUser.full_name[0].toUpperCase() : selectedUser.email[0].toUpperCase()}
                                </div>
                                <div className="overflow-hidden">
                                    <div className="text-sm font-bold text-slate-900 dark:text-white truncate">{selectedUser.full_name || "User"}</div>
                                    <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{selectedUser.email}</div>
                                </div>
                            </div>
                            ...
                            {resetStatus === 'error' && (
                                <div className="p-3 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 text-sm rounded-xl flex items-center gap-2 font-medium">
                            ...
                            {resetStatus === 'success' && (
                                <div className="p-3 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-sm rounded-xl flex items-center gap-2 font-bold">
                        ...
                        <div className="p-4 bg-slate-50/50 dark:bg-[#151925]/50 border-t border-slate-100 dark:border-white/5 flex justify-end gap-3">
                            <button
                                onClick={() => setIsResetOpen(false)}
