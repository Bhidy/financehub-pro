...
class LLMExplainerService:
    ...
    async def generate_narrative(
        ...
        f"3. **عمق التحليل ومطابقة الأنماط التاريخية (CRITICAL)**:\n"
        f"   - قارن المؤشرات الحالية بالمتوسطات التاريخية (مثال: 'يتداول عند 11x مقابل متوسطه التاريخي 14x أي بخصم 20%').\n"
        f"   - إذا كان السهم يتداول بخصم تاريخي، يجب ذكر متى حدث ذلك في الماضي وماذا حدث بعد ذلك (مثال: 'في عام 2016 تداول بخصم 40% ثم ارتفع بنسبة X%').\n"
        f"   - اربط الظروف الاقتصادية الكلية الماضية بالبيئة الحالية لمعرفة مسار التعافي نحو متوسط القطاع.\n"
        ...
        f"   - فقرة 2: التقييم والسياق التاريخي - قارن المؤشرات بالمتوسطات التاريخية والقطاعية. يجب تقدير نسبة التعافي بناءً على التاريخ والتنبؤ بمسار اللحاق بمتوسط القطاع.\n"
        f"   - فقرة 3: النظرة المستقبلية - ما يجب أن يحدث لتحقق السيناريو الإيجابي/السلبي، مع الإشارة للتشابهات مع الدورات الاقتصادية السابقة ومحفزات السوق.\n\n"
        ...
        f"[/THOUGHT_PROCESS]\n"
            ...
            system_prompt = (
                ...
                f"5. **HISTORICAL PATTERN MATCHING** (CRITICAL): If a stock is trading at a historical discount, you MUST:\n"
                f"   - Mention past instances when it traded at similar discounts.\n"
                f"   - Quantify what happened next (e.g., 'In 2016, it traded at a 40% discount and subsequently rallied by X%').\n"
                f"   - Relate those past macroeconomic circumstances to the current environment.\n"
                f"   - Discuss the trajectory required to catch up with sector averages.\n"
        ...
        f"   - Para 2: Valuation Setup & Historical Context — compare current metrics to historical averages and peer averages. You MUST quantify past recoveries (e.g., 'previously rallied X% from this level') and predict the trajectory to catch up with the sector average.\n"
                f"   - Para 3: Forward View — what needs to happen for bull/bear case to play out. Include macroeconomic parallels to past cycles and timing considerations (e.g., seasonal patterns, upcoming catalysts).\n\n"
